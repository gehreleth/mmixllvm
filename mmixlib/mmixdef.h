#ifndef MMIX_DEF_INCLUDED
#define MMIX_DEF_INCLUDED
...
typedef uint8_t MXUByte;

typedef uint16_t MXUWyde;

typedef uint32_t MXUTetra;

typedef uint64_t MXUOcta;

typedef int8_t MXSByte;

typedef int16_t MXSWyde;

typedef int32_t MXSTetra;

typedef int64_t MXSOcta;

typedef double MXFp;

typedef float MXFps;

enum Segments : MXUOcta {
  TEXT_SEG = 0x0000000000000000ULL,
  DATA_SEG = 0x2000000000000000ULL,
  POOL_SEG = 0x4000000000000000ULL,
  STACK_SEG = 0x6000000000000000ULL
};

enum { LOCAL_REG_POW = 10 };

enum class MmixOpcode : MXUByte {
  //#0			  #1			   #2			  #3			  #4			   #5			  #6			  #7
...
#pragma pack(push,1)
union ArithFlag {
  ArithFlag() { memset(this, 0, sizeof(*this)); }
  struct {
    bool FX : 1;
    bool FZ : 1;
    bool FU : 1;
    ...
    bool ED : 1;
    bool : 0;
    unsigned roundMode : 2;
  } bits;
  MXUOcta val;
  bool checkFlags(MXUByte flags) {
    return (flags & val) != 0;
  }
  bool checkEvents(MXUByte events) {
    return ((events << 8) & val) != 0;
  }
};
#pragma pack(pop)
